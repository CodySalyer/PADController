...
#define xThresh 25				// creates a dead zone for the joystick x
#define x0 507					// When x is unmoved
#define xMax 758				// Max x value reading from raw data
#define xMin 232					// Min x value reading from raw data
#define yThresh 25				// creates a dead zone for the joystick y
#define y0 512					// When y is unmoved
#define yMax 796				// Max y value reading from raw data
#define yMin 265					// Min y value reading from raw data
#define thetaThresh 20			// creates a dead zone for the joystick theta
#define theta0 56				// When y is unmoved
#define thetaMax 620			// Max y value reading from raw data
#define thetaMin 0				// Min y value reading from raw data
...
#define REVERSE 0				// This will put out a voltage of 0V for the E~TECH Motor
	...
	aaron X = 127;						// X variable start at Neutral
	aaron Y = 127;						// Y variable start at Neutral
	aaron Theta = 127;					// Theta variable start at Neutral
...
public:
	...
	void readJoystick(void) {
		...
		X = X >= x0 ?								// if Statement 
			map(X, x0, xMax, NEUTRAL, REVERSE) :	// if True 
			map(X, x0, xMin, NEUTRAL, FORWARD);		// if False 
		// Checks x dead-zone
		if (abs(Y - y0) < yThresh) {
			Y = y0;
		}
		Y = Y >= y0 ?								// if Statement
			map(Y, y0, yMax, NEUTRAL, REVERSE) :    // if True
			map(Y, y0, yMin, NEUTRAL, FORWARD);     // if False
		// Checks x dead-zone
		if (abs(Theta - theta0) < thetaThresh) {
			Theta = theta0;
		}
		Theta = Theta >= theta0 ?					// if Statement
			map(Theta, theta0, thetaMax, NEUTRAL, REVERSE) :
			map(Theta, theta0, thetaMin, NEUTRAL, FORWARD);
	...
	void computeMagnitude(void) {
		...
		/// _________________________________________________________________________
		double a = atan2(Y - NEUTRAL, X - NEUTRAL);
		if (Theta != NEUTRAL) {
	...
	void computeAngle(void) {
		...
		if (Theta > NEUTRAL) {
			...
			angle = atan((Y-NEUTRAL)/( X - NEUTRAL));
		}
	}
